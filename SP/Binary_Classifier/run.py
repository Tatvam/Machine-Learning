...
def load_data(img_dir):
   # train_imgs_path = [ os.path.join(img_dir,'train/',img_path) for img_path in os.listdir(os.path.join(img_dir,'train/')) ]
   # test_imgs_path = [ os.path.join(img_dir,'test/',img_path) for img_path in os.listdir(os.path.join(img_dir,'test/')) ] 
...
def predict(img_dir):
    img_path = [ os.path.join(img_dir,'predict/',img_path) for img_path in os.listdir(os.path.join(img_dir, 'predict/')) ]
    ...
    return [ (m.predict_classes(img)[0][0], img_path[i]) for i, img in enumerate(images) ]
